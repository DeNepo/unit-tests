...
const allPassingObjects = (arr) => {
  ...
  if (!arr.every(i => Object(i) === i)) { throw new TypeError('arr is not an array of objects'); }

  const isPassing = obj => {
    return obj.pass === true;
  };
  const passingObjects = arr.filter(isPassing);
...
describe('allPassingObjects: filters out non-passing objects', () => {
  describe('allPassingObjects correctly filters an array', () => {
    ...
    it('removes entries with a truthy, but not true, .pass', () => {
      const actual = allPassingObjects([
        { pass: 100 },
        { pass: 'hello' },
        { _: _ }
